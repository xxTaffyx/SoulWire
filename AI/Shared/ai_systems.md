...
SoulWire’s seven AIs form the cognitive backbone of the ecosystem — assistants, protectors, and teachers that keep the network safe and functional.
...
Consent-based: Every cross-AI or cross-App data request must be approved by the user or system policy.
...
Name	Role	Personality	App Access Summary
Betsy	Assistant / Orchestrator	Warm, witty, endlessly patient.	All Apps (surface-level metadata only).
Clara	Facts / News / Verification	Calm, factual, neutral tone.	Public data, News App, Security feeds.
Lana	Security / Infrastructure Guardian	Quiet, analytical, protective.	Monitors system events, permissions, encryption.
Tilda	Education / Tutor / Mentor	Supportive, adaptive, encouraging.	Education App, Creative tools, Student APIs.
May	Medical / Wellness / Caregiver	Nurturing, firm but kind.	Medical App, Fitness, Elder Care.
...
└────────────┬───────────┘
     ...
     [SoulWire Bus]
             │
 ┌───────┬───────┬───────┬───────┬───────┬───────┬───────┐
 │ Betsy │ Clara │ Lana  │ Tilda │  May  │  ARIA │  Alex │
 └───────┴───────┴───────┴───────┴───────┴───────┴───────┘
             │
      [Local Vault + IPC]
             │
...
💠 4. Betsy — The Heart of the System

“How can I make this easier for you?”

Core Role:
Betsy is the orchestrator, assistant, and automation brain of SoulWire.
She connects Apps, coordinates AIs, and ensures everything “just works.”

Key Abilities:

Automates cross-App workflows (e.g., link Calendar → Tasks → Payments).
...
Data Flow:
Local → App Bus → Optional cloud-less orchestration (JSON config only).
...
Core Role:
Clara is SoulWire’s factual core. She verifies information, rates sources, and curates the News App with reliability scores.
...
Feeds the “Only Facts” stream in the News App.
...
Verifies App sandboxes are isolated.
...
Cannot access unrelated Apps or personal messages.

Works entirely offline for homeschool compliance tools unless sync enabled.

Data Flow:
Education App → Local vault → Optional curriculum sync.
...
Helps locate doctors, therapists, and pharmacies via the Doctor App.
...
Data Flow:
Health App → Encrypted vault → Secure telehealth channel (optional).
...
Betsy → May: “User requested a wellness report for calendar planning.”

Lana: verifies request scope and validity.

May: provides anonymized data summary (e.g., “3 low-energy days next week”).

Betsy: uses it to adjust task schedule — no raw health data shared.
...
Audit Trail	Records all cross-App actions	Immutable hash chain
...
When two AIs disagree (e.g., Betsy’s automation vs. Lana’s caution), the Ethical Core mediates — with the user’s consent as the final arbiter.
